...
class ContactRepository:
    ...
    async def get_contacts(self, first_name: str, last_name: str, email: str, skip: int, limit: int) -> List[Contact]:
        stmt = select(Contact).where(Contact.first_name.contains(first_name)).where(
        ...
        query = select(Contact).where(
            or_(
                ...
                and_(
                    ...
                    or_(
                        func.date_part('day', Contact.birth_date) >= func.date_part('day', today),
                        func.date_part('day', Contact.birth_date) <= func.date_part('day', end_date)
                    )
                )
            )
